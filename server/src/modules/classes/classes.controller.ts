...
export async function getMySessionsV2(req: Request, res: Response) {
  try {
    ...
    const week  = rawWeek  && /^\d{4}-\d{2}-\d{2}$/.test(rawWeek)  ? rawWeek  : undefined;
    const timeZone = (() => {
    ...
    const sessions = await classesService.getMySessionsV2(userId, userRole, date, month, timeZone, week);
...
export async function startSessionById(req: Request, res: Response) {
  try {
    ...
    if (err.message === 'TOO_EARLY') {
      ...
      return res.status(403).json({ error: `Too early to start this session.${mins} Sessions can be started 30 minutes before scheduled time.` });
    }
...
export async function getSubjectStudentsHandler(req: Request, res: Response) {
  try {
    ...
    const students = await getSubjectStudents(subjectId, teacherId);
