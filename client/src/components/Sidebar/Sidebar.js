    ...
    >
      <Container fluid>
        ...
        <Collapse navbar isOpen={collapseOpen}>
          ...
          <hr className="my-3" />
          <h6 className="navbar-heading text-muted sidebar-section-title" style={mini ? { display: "none" } : undefined}>
            <span className="section-dot" style={{ background: "#2dce89" }} />Resources
          </h6>
          <Nav className="mb-md-3" navbar>
            {[
              { href: "#", label: "Course Guide",    icon: "ni ni-spaceship" },
              { href: "#", label: "Student Support",  icon: "ni ni-palette" },
              { href: "#", label: "Help Center",      icon: "ni ni-ui-04" },
            ].map((r) => (
              <NavItem key={r.label} className="sidebar-nav-item">
                <NavLink href={r.href} title={r.label} className="sidebar-link">
                  <i className={r.icon} />
                  <span className="nav-link-text" style={mini ? { display: "none" } : undefined}>{r.label}</span>
                </NavLink>
              </NavItem>
            ))}
          </Nav>
